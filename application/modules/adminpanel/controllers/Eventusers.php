...
class Eventusers extends CI_Controller {
	...
	public function add() {
		...
		if ($this->input->post('action') == "Add") {
			...
			if (($this->input->post('appointment_date') != "" && $this->input->post('appointment_date') != "0000-00-00") && ($this->input->post('appointment_start_time') != "" && $this->input->post('appointment_start_time') != "00:00" ) && ($this->input->post('appointment_end_time') !="" && $this->input->post('appointment_end_time') != "00:00" )) {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if (empty($check_exist)) {
					...
					$insert_data = array(
							...
							"location_date" => $this->input->post('location_date') ? $this->input->post('location_date') :'',
	...
	public function get_appointment_time() {
		...
		if ($appointment_date) {
			...
			$records = $this->Mydb->custom_query("SELECT t1.name AS user_name, t1.email, t2.trip_name, TIME_FORMAT(t1.appointment_start_time , '%H:%i') AS start_time , TIME_FORMAT(t1.appointment_end_time , '%H:%i') AS end_time
					...
					WHERE t1.appointment_date = '" . $appointment_date . "' AND t1.appointment_start_time != t1.appointment_end_time  AND t1.is_delete != '1' AND t1.is_active=1");
	...
	public function check_available_time() {
		...
		if (!empty($appointment_date) && !empty($start_time) && !empty($end_time)) {
			...
			$records = $this->Mydb->custom_query("SELECT * FROM $this->table AS t1 WHERE "
					...
					. " AND t1.appointment_date = '$appointment_date' AND t1.is_delete != '1' $where_edit_id AND t1.is_active=1");
