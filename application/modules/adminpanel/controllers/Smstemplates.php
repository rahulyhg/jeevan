...
class Smstemplates extends CI_Controller {
	public function __construct() {
		...
		$this->module = "smstemplates";
		$this->module_label = "SMS Template";
		$this->module_labels = "SMS Templates";
		$this->folder = "smstemplates/";
		$this->table = "sramcms_sms_templates";	
	...
	function add(){
		...
		if ($this->input->post ( 'action' ) == "Add") {
			...
			$this->form_validation->set_rules ( 'sms_variables', 'lang:sms_variables', 'required' );
			
			if ($this->form_validation->run () == TRUE) {	
				$slug = create_pageuri(post_value ( 'name' ), $this->table, 'slug',array('is_delete !=' => 1));
				$insert_array = array (
						'name' 			=> post_value ( 'name' ), 
						'sms_content' 	=> post_value ( 'sms_content' ),
						'sms_variables'	=> post_value('sms_variables'),
						'slug'			=> $slug,	
						'is_active' 	=> '1',
