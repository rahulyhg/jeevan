...
class Blog extends CI_Controller {
	public function __construct() {
		...
		$this->module_label = "Blog Page";
		$this->module_labels = "Blog Page (s)";
		$this->folder = "blog/";
		$this->table = "blog";	
	...
	function add(){
		...
		if ($this->input->post ( 'action' ) == "Add") {
			...
			$this->form_validation->set_rules ( 'blog_title', 'lang:page_title', 'required' );
			$this->form_validation->set_rules ( 'author', 'lang:author', 'required' );
			$this->form_validation->set_rules ( 'blog_shortdesc', 'lang:blog_shortdesc', 'required' );
			$this->form_validation->set_rules ( 'blog_image', 'lang:blog_image', 'callback_validate_page_image' );
			...
			if ($this->form_validation->run () == TRUE) {	
				...
				{					
					$create_blog_image = md5(uniqid(rand())).'-blog-'.$_FILES['blog_image']['name'];
					$blog_image = $this->common->upload_image ( 'blog_image', $create_blog_image,'blog/' );
					
						
				} else {
					$blog_image = '';
				}
				$slug = create_pageuri(post_value ( 'blog_title' ), $this->table, 'blog_slug',array('is_delete !=' => 1));
				$insert_array = array (
						'blog_title' => post_value ( 'blog_title' ),
						'blog_shortdesc' => post_value ( 'blog_shortdesc' ),
						'blog_slug' => $slug,
						'blog_description' => $this->input->post('blog_description', false),
						'author' => post_value('author'),
	...
	public function edit($edit_id = NULL) {
		...
		if ($this->input->post ( 'action' ) == "edit") {
			...
			$this->form_validation->set_rules ( 'blog_title', 'lang:blog_title', 'required' );
			$this->form_validation->set_rules ( 'blog_shortdesc', 'lang:blog_shortdesc', 'required' );
			$this->form_validation->set_rules ( 'author', 'lang:author', 'required' );
			$this->form_validation->set_rules ( 'blog_image', 'lang:blog_image', 'callback_validate_page_image' );
			...
			if ($this->form_validation->run () == TRUE) {
				...
				$slug = create_pageuri(post_value ( 'blog_title' ), $this->table, 'blog_slug',array('is_delete !=' => 1, 'id !=' => $id));
				...
				{
					$create_blog_image = md5(uniqid(rand())).'-blog-'.$_FILES['blog_image']['name'];
					$blog_image = $this->common->upload_image ( 'blog_image', $create_page_image,'blog/' );
					$image_arr = array (
							'blog_image' => $blog_image
					);
	...
	public function validate_page_image() {
		if (isset ( $_FILES ['blog_image'] ['name'] ) && $_FILES ['blog_image'] ['name'] != "") {
			if ($this->common->valid_image ( $_FILES ['blog_image'] ) == "No") {
