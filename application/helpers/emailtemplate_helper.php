...
{
	function send_email($to_email_address,$template_slug,$chk_arr = array(),$rep_arr =array(),$attach_file=array(),$path='',$subject='',$cc='', $html_template = 'email_template')
	{
		
		 $CI =  & get_instance();
		 $template_table = "sramcms_email_templates";
		 $setting_table  = "site_settings";
		 
     	 $query = "SELECT from_email , reply_to, name, description, subject, email_content, email_variables FROM  $template_table WHERE slug = '".$template_slug."'";
         ...
         if(!empty($result))
         {
         	...
         	/* get basic mail config values */
           	 ...
           	 /*echo "<pre>";
           	   ...
           	   $chk_arr1 = array('[BASEURL]','[COPY-CONTENT]','[ADMIN-EMAIL]','[SITE-TITLE]','[SITEURL]');
           	   $rep_array2 = array(base_url(),'JEEVANACHARYA © 2017. ALL RIGHT RESERVED.',$CI->config->item('from_email', 'siteSettings'),'JEEVANACHARYA',BASE_URL());
