define(["jquery", "knockout", "gmaps", "eventEmitter", "config", "layer", "toolbar", "shape", "destination"], function ($, ko, gmaps, EventEmitter, config, Layer, Toolbar, Shape, Destination) {
    ...
    var Shape = function (options) {
        ...
        this.startdate = ko.observable(options.startdate);
        ...
        if (this.type() == "directions") {
            ...
            this.totalDurationPrint = ko.computed(function () {
                ...
                if (vHours > 0)
                    return vHours + " hours " + Math.floor(v % 3600 / 60) + " mins";
    ...
    Shape.prototype.draw = function () {
        ...
        if (this.type() == "directions" && this.map()) {
            var callback = function () {
                ...
                if (self.preserveViewport)
                    self.directionsModel.setOptions({preserveViewport: self.preserveViewport});
                self.directionsModel.setMap(self.map());
                self.directionsModel.setDirections(self.data);
            }
            if (this.directionsDataIsReady)
                callback();
            else
                this.buildDirections(callback);
        }
    }
    ...
    Shape.prototype.exportShape = function () {
        var exportData = {
            ...
            type: this.type(),
            startdate: this.layer.startdate(),
            enddate: this.layer.enddate(),
        }
        if (this.type() == "marker") {
